...
from ..utils.documents import is_url, download_document, get_local_document
...
async def handle_summarize(
    ...
    try:
        ...
        # Extract text from pages
        pages = await _extract_page_text(doc_path, {"range": page_range} if page_range else None)
...
async def _extract_page_samples(doc_path: Path, page_range: Optional[Dict[str, int]] = None) -> List[Dict[str, Any]]:
...
async def _extract_page_text(doc_path: Path, page_filter: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
